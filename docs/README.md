---
home: true
heroImage: http://resource.muyiy.cn/image/javascript.gif
actions:
- text: 项目简介
  link: /guide.html
  type: primary
- text: 良菌的bilibili(技术无关)
  link: https://space.bilibili.com/38565480
  type: secondary
features:
- title: 热爱技术
  details: 俗话说,兴趣才是最好的老师.尽量多折腾折腾是最好的提升路径.
- title: 费曼学习法
  details: 要以把一个知识点总结讲明白给一个保洁阿姨为目标.
- title: 积极分享
  details: 好记性不如烂笔头.就在这里把自己总结的分享出来.
- title: 技术驱动
  details: 新的技术如同新的营养,赶快吸取一下.
- title: 保持求知欲
  details: Stay hungry,stay foolish.
- title: 爱好
  details: 一个兴趣广泛滴程序员,B站(Earth良菌)可以看看我技术以外的兴趣哦.在这里,除了技术,什么都搞.
footer: ISC Licensed | Copyright © 2021-present ChenLiangjun
---